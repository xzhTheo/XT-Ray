...
// Plane
Plane::Plane(const glm::vec3& aPoint, const glm::vec3& normal) : _normal(glm::normalize(normal)), _aPoint(aPoint), Shape("Plane")
{
...
{
    float v1 = glm::dot(ray.getOrigin() - _aPoint, _normal);
...
bool Triangle::inTriangle(const glm::vec3& p) const
{
    ...
    for (int i = 0; i < 3; i++)
    {
        glm::vec3 triangleEdge = _vertice[(i+1)%3] - _vertice[i]; // 计算边向量
...
// Sphere
Sphere::Sphere(const glm::vec3& center, float radius) : _center(center), _radius(radius), Shape("Sphere")
{
...
{
    glm::vec3 vc = ray.getOrigin() - _center;
...
{
    return inSphere(ray.getOrigin()) && rayCollision(ray) > FLOAT_EPS;
}
